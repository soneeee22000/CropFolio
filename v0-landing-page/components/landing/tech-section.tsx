...
export function TechSection() {
  ...
  return (
    <section 
      ref={ref}
      className="bg-background border-t border-border py-16 sm:py-20 px-6"
    >
      <div className="max-w-4xl mx-auto">
        {/* Header */}
        <p 
          className={`text-[11px] uppercase tracking-[0.25em] text-muted-foreground mb-8 text-center font-sans transition-all duration-700 ${
        ...
        >
          {techStack.map((tech, index) => (
            <div 
              key={tech.name}
              className="px-4 py-2 bg-secondary border border-border rounded-full font-mono text-sm text-foreground transition-all hover:border-primary/30 hover:bg-primary/5"
